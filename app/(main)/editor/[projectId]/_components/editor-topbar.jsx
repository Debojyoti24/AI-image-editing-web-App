...
export function EditorTopBar({ project }) {
  ...
  // ✅ Unlock listener (important)
  useEffect(() => {
    const handleUnlock = (e) => {
      const unlockedTool = e.detail.tool;
      console.log("✅ Free tool unlocked:", unlockedTool);

      // Close modal and reset
      setShowUpgradeModal(false);
      setRestrictedTool(null);

      // Automatically switch to unlocked tool
      if (unlockedTool) onToolChange(unlockedTool);
    };

    window.addEventListener("unlockProFeature", handleUnlock);
    return () => window.removeEventListener("unlockProFeature", handleUnlock);
  }, [onToolChange]);
  ...
  useEffect(() => {
    ...
    const handleCanvasModified = () => {
      if (!isUndoRedoOperation) setTimeout(() => saveToUndoStack(), 500);
    };
  ...
  const handleManualSave = async () => {
    if (!canvasEditor || !project) return toast.error("Canvas not ready");
    try {
      const canvasJSON = canvasEditor.toJSON();
  ...
  const handleExport = async (exportConfig) => {
    ...
    try {
      const currentZoom = canvasEditor.getZoom();
      const currentTransform = [...canvasEditor.viewportTransform];
      canvasEditor.setZoom(1);
      canvasEditor.setViewportTransform([1, 0, 0, 1, 0, 0]);
      canvasEditor.setDimensions({
        width: project.width,
      ...
      canvasEditor.setZoom(currentZoom);
      canvasEditor.setViewportTransform(currentTransform);
      const link = document.createElement("a");
      link.download = `${project.title}.${exportConfig.extension}`;
      link.href = dataURL;
      link.click();
  ...
  const handleResetToOriginal = async () => {
    if (!canvasEditor || !project?.originalImageUrl) {
      return toast.error("No original image");
    }
    saveToUndoStack();
    try {
      canvasEditor.clear();
      ...
      <div className="border-b px-6 py-3">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="glass" size="sm" disabled={isExporting}>
                  {isExporting ? (
                    <>
                      <Loader2 className="h-4 w-4 animate-spin" />
                      Exporting {exportFormat}...
            ...
            {TOOLS.map((tool) => {
              ...
              return (
                <Button
                  ...
                  size="sm"
                  onClick={() => handleToolChange(tool.id)}
                  className={`${isActive ? "bg-blue-600 text-white" : "text-white"} ${
                    !hasToolAccess ? "opacity-60" : ""
                  }`}
                >
                  <Icon className="h-4 w-4" />
